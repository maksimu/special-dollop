...
use guacr_terminal::{
    AsciicastHeader, DualFormatRecorder,
};
...
fn test_asciicast_format_compliance() {
    ...
    for line in &lines[1..] {
        ...
        assert!(arr[0].is_f64() || arr[0].is_u64(), "Timestamp should be numeric");
        
        // Validate type is 'o' for output
        assert_eq!(arr[1].as_str().unwrap(), "o", "Type should be 'o' for output");
...
fn test_unicode_recording() {
    ...
    recorder.record_output("مرحبا بالعالم\n".as_bytes()).unwrap(); // Arabic
    recorder.record_output("🚀 Emoji test\n".as_bytes()).unwrap(); // Emoji
    ...
    assert!(content.contains("世界"), "Chinese characters should be preserved");
    assert!(content.contains("Привет"), "Russian characters should be preserved");
    assert!(content.contains("مرحبا"), "Arabic characters should be preserved");
...
fn test_ses_format() {
    ...
    for line in lines {
        ...
        assert_eq!(parts.len(), 3, "Each line should have timestamp.direction.instruction");
        
        // Validate timestamp is numeric
        parts[0].parse::<u64>().expect("Timestamp should be numeric");
...
fn test_dual_format_recording() {
    ...
    assert!(fs::metadata(&cast_path).unwrap().len() > 0, "asciicast should have content");
    assert!(fs::metadata(&ses_path).unwrap().len() > 0, ".ses file should have content");
}
...
fn test_resize_recording() {
    ...
    assert!(content.contains("\"r\""), "Should contain resize event type");
}
...
fn test_large_output_recording() {
    ...
    for i in 0..1000 {
        recorder.record_output(format!("Line {}\n", i).as_bytes()).unwrap();
    }
    recorder.finalize().unwrap();
...
fn test_empty_recording() {
    ...
    let recorder = DualFormatRecorder::new(
        Some(&cast_path),
        None,
        80,
        24,
