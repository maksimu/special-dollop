    ...
    def setUp(self):
        ...
        with BaseWebRTCTest._registry_lock:
            if BaseWebRTCTest._shared_registry is None:
                BaseWebRTCTest._shared_registry = keeper_pam_connections.PyTubeRegistry()
...
def init_logger():
    ...
    if not RUST_LOGGER_INITIALIZED:
        try:
            keeper_pam_connections.initialize_logger("keeper_pam_connections", verbose=True, level=logging.DEBUG)
