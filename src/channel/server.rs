                ...
                match listener_clone.accept().await {
                    Ok((stream, peer_addr)) => {
                        ...
                        if !is_localhost {
                            if active_protocol == ActiveProtocol::Socks5 {
                                ...
                                if let Ok(_) = reader.read_exact(&mut buf).await {
                                    ...
                                    if version == SOCKS5_VERSION {
                                        ...
                                        if let Err(e) = writer.write_all(&[SOCKS5_VERSION, SOCKS5_AUTH_FAILED]).await {
                            ...
                            error!("Channel({}): Connection from non-localhost address rejected", channel_id);
                            ...
                            ActiveProtocol::Socks5 => {
                                ...
                                tokio::spawn(async move {
                                    if let Err(e) = handle_socks5_connection(
                                        stream, current_conn_no, 
                                        conn_tx_clone, webrtc_clone, buffer_pool_clone, task_channel_id
                                    ).await {
                                        error!("Channel({}): SOCKS5 connection error: {}", error_log_channel_id, e);
                                    }
                                });
                            },
                            ActiveProtocol::PortForward | ActiveProtocol::Guacd => {
                                ...
                                tokio::spawn(async move {
                                    if let Err(e) = handle_generic_server_connection(
                                        stream, current_conn_no, active_protocol,
                                        conn_tx_clone, webrtc_clone, buffer_pool_clone, task_channel_id
                                    ).await {
    ...
    pub async fn stop_server(&mut self) -> Result<()> {
        if let Some(task) = self.local_client_server_task.take() {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(5), task).await {
                ...
                Err(_) => {
                    warn!("Endpoint {}: Server task did not shutdown in time", self.channel_id);
                }
            }
        }
...
async fn handle_socks5_connection(
    stream: TcpStream,
    conn_no: u32,
    conn_tx: tokio::sync::mpsc::Sender<(u32, tokio::net::tcp::OwnedWriteHalf, tokio::task::JoinHandle<()>)>,
    ...
    if socks_version != SOCKS5_VERSION {
        error!("Channel({}): Invalid SOCKS version: {}", channel_id, socks_version);
        writer.write_all(&[SOCKS5_VERSION, SOCKS5_AUTH_FAILED]).await?;
    ...
    debug!("Channel({}): SOCKS5 connection to {}:{}", channel_id, dest_host, dest_port);
        ...
        loop {
            ...
            match reader.read(read_slice).await {
                ...
                Ok(n) => {
                    ...
                    // Create a data frame
                    let frame = Frame::new_data_with_pool(conn_no, &read_buffer[0..n], &buffer_pool_clone);
                    ...
                    if let Err(e) = dc.send(encoded).await {
                        error!("Channel({}): Failed to send data to tunnel: {}", endpoint_name, e);
                        break;
                    }
                },
                Err(e) => {
                    error!("Channel({}): Error reading from client: {}", endpoint_name, e);
                    break;
                }
            }
        }
    ...
    conn_tx.send((conn_no, writer, read_task)).await
...
async fn handle_generic_server_connection(
    ...
    debug!("Channel({}): New generic {:?} connection {}", channel_id, active_protocol, conn_no);
    ...
    let open_frame = Frame::new_control_with_pool(ControlMessage::OpenConnection, &open_conn_payload, &buffer_pool);
    webrtc.send(open_frame.encode_with_pool(&buffer_pool)).await
        .map_err(|e| anyhow!("Failed to send OpenConnection for new server stream {}: {}", conn_no, e))?;

    debug!("Channel({}): Sent OpenConnection for new server stream {} ({:?})", channel_id, conn_no, active_protocol);
            ...
            match reader.read(read_slice).await {
                Ok(0) => {
                    ...
                    if let Err(e) = dc_clone.send(eof_frame.encode_with_pool(&buffer_pool_clone)).await {
                        error!("Channel({}): Failed to send EOF for conn_no {}: {}", channel_id_clone, conn_no, e);
                    }
                    break; 
                }
                Ok(n) if n > 0 => {
                    ...
                    let data_frame = Frame::new_data_with_pool(conn_no, current_payload_bytes_slice, &buffer_pool_clone);
    ...
    conn_tx.send((conn_no, writer, read_task)).await
        .map_err(|e| anyhow!("Failed to send new server connection {} to channel: {}", conn_no, e))?;
