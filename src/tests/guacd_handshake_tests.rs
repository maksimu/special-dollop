...
use crate::channel::guacd_parser::{GuacdInstruction, GuacdParser, PeekError, INST_TERM, ARG_SEP, ELEM_SEP};
...
async fn create_test_rtc_data_channel(label: &str) -> Arc<ActualRTCDataChannel> {
    ...
    pc.create_data_channel(label, Some(data_channel_init)).await.unwrap()
}
...
async fn test_guacd_handshake_successful_new_connection() {
    ...
    protocol_settings.insert("guacd".to_string(), serde_json::json!({
        "guacd_host": "localhost", 
        "guacd_port": 4822
    }));
    let guacd_params_for_channel = guacd_params.clone();
    protocol_settings.insert("guacd_params".to_string(), serde_json::json!(guacd_params_for_channel));
    ...
    let server_task = tokio::spawn(async move {
        ...
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server: Failed to get select instruction");
        assert_eq!(received_select.opcode, "select");
        assert_eq!(received_select.args, vec!["rdp"]);
        println!("Server (success_new): Received select: {:?}", received_select);
        
        let args_response = encode_guac_instruction("args", &["VERSION_1_5_0", "hostname", "port", "username", "password", "domain"]);
        server_stream.write_all(&args_response).await.expect("Server failed to send args");
        ...
        for i in 0..20 { // Try to read multiple times
            match tokio::time::timeout(Duration::from_millis(250), server_stream.read(&mut read_buf_array)).await {
                Ok(Ok(0)) => {
                    println!("Server (success_new): V2: read EOF. Total data: {}. Loop iter: {}", diagnostic_buffer.len(), i);
                    break;
                }
                Ok(Ok(n)) => {
                ...
                Ok(Err(e)) => {
                    println!("Server (success_new): V2: read error: {}. Loop iter: {}", e, i);
                    break;
                }
                Err(_) => { // Timeout from tokio::time::timeout
                    ...
                    if connect_received { break; } // Exit if connect was found before timeout
                    if i == 19 { println!("Server (success_new): V2: Max read attempts with timeouts.");}
                }
            }
        }
        ...
        server_stream.write_all(&ready_response).await.expect("Server failed to send ready");
    ...
    let handshake_result = timeout(handshake_timeout_duration, crate::channel::connections::perform_guacd_handshake(
        &mut client_reader,
        &mut client_writer,
        &channel_id_clone,
        conn_no,
...
async fn test_guacd_handshake_join_existing_connection_readonly() {
    ...
    protocol_settings.insert("guacd_params".to_string(), serde_json::json!(guacd_params.clone()));
    let channel = Channel::new(
        mock_webrtc_dc_for_channel,
        dc_rx_for_channel,
    ...
    let conn_no = 2u32;
    let server_task = tokio::spawn(async move {
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server: Failed to get select instruction for join");
        assert_eq!(received_select.opcode, "select");
        ...
        server_stream.write_all(&args_response).await.expect("Server: Failed to send args for join");
        println!("Server (join): Sent args: {:?}", server_supported_args);
        let received_connect = recv_and_get_instruction(&mut server_stream).await.expect("Server: Failed to get connect instruction for join");
        assert_eq!(received_connect.opcode, "connect");
        assert_eq!(received_connect.args, vec!["VERSION_1_5_0".to_string(), "".to_string(), "true".to_string(), "".to_string()]);
        println!("Server (join): Received connect: {:?}", received_connect);
        let ready_response = encode_guac_instruction("ready", &["joined-client-id"]);
        server_stream.write_all(&ready_response).await.expect("Server: Failed to send ready for join");
    ...
    server_task.await.expect("Server task (join_readonly) panicked");
}
...
async fn test_guacd_handshake_join_existing_connection_not_readonly() {
    ...
    let conn_no = 3u32;
    let server_task = tokio::spawn(async move {
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server (nr): Failed to get select");
        assert_eq!(received_select.opcode, "select");
        assert_eq!(received_select.args, vec![connection_id_to_join]);
        println!("Server (join_nr): Received select: {:?}", received_select);
        let server_supported_args = vec!["VERSION_1_5_0", "param-a", "read-only", "param-b"];
        let args_response = encode_guac_instruction("args", &server_supported_args);
        server_stream.write_all(&args_response).await.expect("Server (nr): Failed to send args");
        println!("Server (join_nr): Sent args: {:?}", server_supported_args);
        let received_connect = recv_and_get_instruction(&mut server_stream).await.expect("Server (nr): Failed to get connect");
        assert_eq!(received_connect.opcode, "connect");
        assert_eq!(received_connect.args, vec!["VERSION_1_5_0".to_string(), "".to_string(), "".to_string(), "".to_string()]);
        ...
        server_stream.write_all(&ready_response).await.expect("Server (nr): Failed to send ready");
...
async fn test_guacd_handshake_failure_wrong_opcode_instead_of_args() {
    ...
    let conn_no = 4u32;
    let server_task = tokio::spawn(async move {
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server (fail_args): Failed to get select");
        assert_eq!(received_select.opcode, "select");
        ...
        server_stream.write_all(&unexpected_response).await.expect("Server (fail_args): Failed to send unexpected_opcode");
    ...
    match handshake_result {
        ...
        Ok(Err(e)) => {
            ...
            println!("Client (fail_args): Handshake failed as expected: {}", err_string);
...
async fn test_guacd_handshake_failure_wrong_opcode_instead_of_ready() {
    ...
    let conn_no = 5u32;
    let server_task = tokio::spawn(async move {
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server (fail_ready): Failed to get select");
        assert_eq!(received_select.opcode, "select");
        println!("Server (fail_ready): Received select: {{opcode: '{}', args: {:?}}}", received_select.opcode, received_select.args);
        
        let args_response = encode_guac_instruction("args", &["VERSION_1_5_0", "hostname", "port", "username"]);
        server_stream.write_all(&args_response).await.expect("Server (fail_ready): Failed to send args");
        ...
        for i in 0..20 { // Increased attempts, shorter timeout per attempt
            match tokio::time::timeout(Duration::from_millis(250), server_stream.read(&mut read_buf_array)).await {
                Ok(Ok(0)) => {
                ...
                Ok(Err(e)) => {
                    println!("Server (fail_ready): DIAGNOSTIC V2: read error: {}. Loop iter: {}", e, i);
                    break;
                }
                Err(_) => { // Timeout from tokio::time::timeout
    ...
    match handshake_result {
        ...
        Ok(Err(e)) => {
            let err_string = e.to_string();
            println!("Client (fail_ready): Handshake failed as expected. Error: {}", err_string);
        ...
        Err(elapsed) => { // Timeout error from tokio::time::timeout
    ...
    server_task.await.expect("Server task (fail_ready) panicked");
}
...
async fn test_guacd_handshake_failure_timeout_waiting_for_args() {
    ...
    let conn_no = 6u32;
    let server_task = tokio::spawn(async move {
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server (timeout_args): Failed to get select");
        assert_eq!(received_select.opcode, "select");
        println!("Server (timeout_args): Received select: {:?}", received_select);
    ...
    match handshake_result {
        ...
        Ok(Err(e)) => {
            println!("Client (timeout_args): Handshake failed with specific error (e.g. EOF): {}", e);
            assert!(e.to_string().contains("EOF during Guacd handshake while waiting for 'args'"));
        }
        Err(_) => {
...
async fn test_guacd_handshake_failure_timeout_waiting_for_ready() {
    ...
    let mut guacd_params = HashMap::new();
    guacd_params.insert("protocol".to_string(), "rdp".to_string());
    guacd_params.insert("hostname".to_string(), "testserver_timeout_ready".to_string());
    guacd_params.insert("port".to_string(), "3389".to_string());
    guacd_params.insert("width".to_string(), "1024".to_string());
    guacd_params.insert("height".to_string(), "768".to_string());
    guacd_params.insert("dpi".to_string(), "96".to_string());
    ...
    let server_task = tokio::spawn(async move {
        ...
        let received_select = recv_and_get_instruction(&mut server_stream).await.expect("Server (timeout_ready): Failed to get select");
        assert_eq!(received_select.opcode, "select");
        println!("Server (timeout_ready): Received select: {:?}", received_select);

        let args_response = encode_guac_instruction("args", &["VERSION_1_5_0", "hostname", "port"]);
        server_stream.write_all(&args_response).await.expect("Server (timeout_ready): Failed to send args");
        ...
        for i in 0..20 { // Try to read multiple times
            match tokio::time::timeout(Duration::from_millis(250), server_stream.read(&mut read_buf_array)).await {
                Ok(Ok(0)) => {
                    println!("Server (timeout_ready): V2: read EOF. Total data: {}. Loop iter: {}", diagnostic_buffer.len(), i);
                    break;
                }
                Ok(Ok(n)) => {
                    diagnostic_buffer.extend_from_slice(&read_buf_array[..n]);
                    println!("Server (timeout_ready): V2: read {} bytes. Total: {}. Loop iter: {}", n, diagnostic_buffer.len(), i);
                ...
                Ok(Err(e)) => {
                    println!("Server (timeout_ready): V2: read error: {}. Loop iter: {}", e, i);
                    break;
                }
                Err(_) => { // Timeout from tokio::time::timeout
                    ...
                    if i == 19 { println!("Server (timeout_ready): V2: Max read attempts with timeouts.");}
                }
            }
        }
        ...
        if !connect_instr_received_and_validated {
            ...
            println!("Server (timeout_ready): V2: All expected instructions up to 'connect' processed.");
        }
    ...
    match handshake_result {
        ...
        Ok(Err(e)) => {
            println!("Client (timeout_ready): Handshake failed with specific error (e.g. EOF): {}", e);
            assert!(e.to_string().contains("EOF during Guacd handshake while waiting for 'ready'"));
        }
        Err(_) => {
