...
fn test_tube_creation() {
    ...
    runtime.block_on(async {
        ...
        let connection_fut = tube.create_peer_connection(None, true, false, "TEST_MODE_KSM_CONFIG_1".to_string(), "TEST_CALLBACK_TOKEN_1".to_string(), settings, signal_tx);
        ...
        let control_channel_fut = tube.create_control_channel("TEST_MODE_KSM_CONFIG_1".to_string(), "TEST_CALLBACK_TOKEN_1".to_string());
...
async fn test_tube_webrtc_connection() {
    ...
    tube.create_peer_connection(
        ...
        settings,
        signal_tx
    ).await.expect("Failed to create peer connection");
...
async fn test_tube_create_channel() {
    ...
    tube.create_peer_connection(
        ...
        settings.clone(),
        signal_tx
    ).await.expect("Tube failed to create peer connection");

    let data_channel = tube.create_data_channel("test_dc", "ksm_config_val".to_string(), "token_val".to_string()).await.expect("Failed to create data channel");
...
async fn perform_signaling_and_ice_exchange(
    ...
    if tube2.peer_connection().await.is_none() {
        return Err("Tube2 peer connection not initialized before set_remote_description".to_string());
    }
    tube2
        .set_remote_description(offer, false)
        .await
        .map_err(|e| format!("[perform_signaling] Tube2 set_remote_description (offer) error: {}", e))?;
    ...
    if tube1.peer_connection().await.is_none() {
        return Err("Tube1 peer connection not initialized before set_remote_description for answer".to_string());
    }
    tube1
        .set_remote_description(answer, true)
        .await
        .map_err(|e| format!("[perform_signaling] Tube1 set_remote_description (answer) error: {}", e))?;
    ...
    for attempt in 1..=max_ice_exchange_attempts {
        info!("[perform_signaling] ICE exchange attempt {}/{}", attempt, max_ice_exchange_attempts);

        let state1_pc = tube1.peer_connection().await.map_or(RTCPeerConnectionState::Unspecified, |p| p.peer_connection.connection_state());
        let state2_pc = tube2.peer_connection().await.map_or(RTCPeerConnectionState::Unspecified, |p| p.peer_connection.connection_state());
        info!("[perform_signaling] PC states: tube1={:?}, tube2={:?}", state1_pc, state2_pc);

        if state1_pc == RTCPeerConnectionState::Connected && state2_pc == RTCPeerConnectionState::Connected {
            ice_connected = true;
            ...
            match tokio::time::timeout(Duration::from_millis(100), signal_rx1.recv()).await {
                Ok(Some(signal)) => {
                    if signal.kind == "icecandidate" {
                        info!("[perform_signaling] Tube1 ICE candidate: {}", if signal.data.is_empty() { "<empty>" } else { &signal.data });
                        // Make sure to add the empty candidate to tube2 as well
                        tube2.add_ice_candidate(signal.data.clone()).await
                            .map_err(|e| format!("[perform_signaling] Tube2 add_ice_candidate error: {}", e))?;
        ...
        while !tube2_ice_candidates_finished {
            match tokio::time::timeout(Duration::from_millis(100), signal_rx2.recv()).await {
                Ok(Some(signal)) => {
                    if signal.kind == "icecandidate" {
                        info!("[perform_signaling] Tube2 ICE candidate: {}", if signal.data.is_empty() { "<empty>" } else { &signal.data });
                        // Make sure to add the empty candidate to tube1 as well
                        tube1.add_ice_candidate(signal.data.clone()).await
                            .map_err(|e| format!("[perform_signaling] Tube1 add_ice_candidate error: {}", e))?;
                        exchanged_any_this_attempt = true;
        ...
        let current_state1_pc = tube1.peer_connection().await.map_or(RTCPeerConnectionState::Unspecified, |p| p.peer_connection.connection_state());
        let current_state2_pc = tube2.peer_connection().await.map_or(RTCPeerConnectionState::Unspecified, |p| p.peer_connection.connection_state());
        if current_state1_pc == RTCPeerConnectionState::Connected && current_state2_pc == RTCPeerConnectionState::Connected {
            ice_connected = true;
        ...
        if !exchanged_any_this_attempt && attempt > 8 { 
            ...
            let final_state1_pc = tube1.peer_connection().await.map_or(RTCPeerConnectionState::Unspecified, |p| p.peer_connection.connection_state());
            let final_state2_pc = tube2.peer_connection().await.map_or(RTCPeerConnectionState::Unspecified, |p| p.peer_connection.connection_state());
            if final_state1_pc == RTCPeerConnectionState::Connected && final_state2_pc == RTCPeerConnectionState::Connected {
                ice_connected = true;
            ...
            // Consider breaking or returning an error if stuck
             if attempt > 15 && (final_state1_pc == RTCPeerConnectionState::Failed || final_state2_pc == RTCPeerConnectionState::Failed) {
                return Err(format!("ICE failed, peer connection state is Failed. T1: {:?}, T2: {:?}", final_state1_pc, final_state2_pc));
            }
        }
...
async fn test_tube_p2p_data_transfer_end_to_end() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tube1.create_peer_connection(Some(rtc_config.clone()), true, false, ksm_config_t1.clone(), token_t1.clone(), settings.clone(), signal_tx1).await?;
    info!("[E2E_TEST] Tube1 peer connection created.");
    tube2.create_peer_connection(Some(rtc_config), true, false, ksm_config_t2.clone(), token_t2.clone(), settings, signal_tx2).await?;
    ...
    info!("[E2E_TEST] Tube1 attempting to create data channel '{}' BEFORE offer", dc_label);
    let dc1_out = tube1.create_data_channel(&dc_label, ksm_config_t1.clone(), token_t1.clone()).await?;
    info!("[E2E_TEST] Tube1: create_data_channel call returned for '{}'", dc_label);

    let (dc1_open_tx, dc1_open_rx) = oneshot::channel();
    let dc1_label_clone_for_onopen = dc1_out.label(); 
    dc1_out.data_channel.on_open(Box::new(move || { // Attach to raw RTCDataChannel
        info!("[E2E_TEST] Tube1: dc1_out '{}' (RTCDataChannel) ON_OPEN triggered.", dc1_label_clone_for_onopen);
    ...
    tokio::time::timeout(Duration::from_secs(20), dc1_open_rx) // Increased timeout for channel open
        .await.map_err(|e| format!("[E2E_TEST] Timeout waiting for dc1_out to open: {}", e))?
        .map_err(|e| format!("[E2E_TEST] dc1_out_open_rx failed: {}", e))?;
    info!("[E2E_TEST] Tube1: dc1_out '{}' is confirmed open.", dc_label);
        ...
        if let Some(found_dc_wrapper) = tube2.get_data_channel(&dc_label) {
            ...
            if found_dc_wrapper.data_channel.ready_state() != RTCDataChannelState::Open {
                info!("[E2E_TEST] Tube2: dc_in '{}' RTCDataChannel not yet open, waiting for test's ON_OPEN...", dc_label);
                 ...
                 info!("[E2E_TEST] Tube2: dc_in '{}' confirmed open via test\\'s ON_OPEN.\"", dc_label);
            } else {
                info!("[E2E_TEST] Tube2: dc_in '{}' was already open when retrieved.\"", dc_label);
            }
            break;
        }
        info!("[E2E_TEST] Tube2: Waiting for data channel '{}'... Attempt {}/150\"", dc_label, i + 1);
        tokio::time::sleep(Duration::from_millis(100)).await;
    }

    let dc2_in = dc2_in_opt.ok_or_else(|| format!("[E2E_TEST] Data channel '{}' not found or set up on tube2\"", dc_label))?;
    // Use data_channel.ready_state() for openness check
    assert_eq!(dc2_in.data_channel.ready_state(), RTCDataChannelState::Open, "[E2E_TEST] dc2_in '{}' should be open after setup loop\"", dc_label);
    info!("[E2E_TEST] Tube2: dc_in '{}' setup complete and confirmed open.\"", dc_label);

    let test_message_string = format!("Hello from Tube1 {} to Tube2 {} at {}", tube1.id(), tube2.id(), Utc::now().to_rfc3339());
    let sent_bytes = Bytes::from(test_message_string.clone()); // Bytes to be sent and used for assertion
    info!("[E2E_TEST] Tube1: Sending message (as Bytes, content: '{}') on dc '{}'", test_message_string, dc1_out.label());
    dc1_out.send(sent_bytes.clone()).await?; // Send a clone of sent_bytes
    info!("[E2E_TEST] Tube1: Message sent.");

    info!("[E2E_TEST] Tube2: Waiting for message on dc '{}'...", dc_label);
    match tokio::time::timeout(Duration::from_secs(15), msg_rx.recv()).await { // msg_rx now receives Bytes
        Ok(Some(received_bytes)) => { // received_bytes is Bytes
            ...
            let received_string_preview = String::from_utf8(received_bytes.to_vec()).unwrap_or_else(|_| "[[not a valid UTF-8 string]]".to_string());
            // sent_bytes was derived from test_message_string, so it should be valid UTF-8.
            let sent_string_preview = String::from_utf8(sent_bytes.to_vec()).expect("sent_bytes should be valid UTF-8");
        ...
        Ok(None) => {
            return Err(Box::from("[E2E_TEST] Tube2: Message channel closed prematurely by sender."));
        }
        Err(e) => {
            return Err(Box::from(format!("[E2E_TEST] Tube2: Timeout waiting for message: {}", e)));
        }
    }
